...
exports.userCreate = functions.database
  ...
  .onCreate(async (snapshot, context) => {
    ...
    console.log(
      `Done syncing firestore user with id: ${userInfo.uid} to Stripe. The stripe id is ${stripeCustomer.id}`
    );
...
exports.user_signup = functions.https.onRequest(async (request, response) => {
  ...
  try {
    ...
    if (userDetails.usertype === "driver") {
      regData.taxiProCardImage = userDetails.taxiProCardImage;
      regData.idImage = userDetails.idImage;
      regData.parkingCardImage = userDetails.parkingCardImage;
      regData.insuranceCertificateImage = userDetails.insuranceCertificateImage;
      regData.certificateRcProImage = userDetails.certificateRcProImage;
      regData.kbisOrInseeImage = userDetails.kbisOrInseeImage;
      regData.registrationCertificateImage =
        userDetails.registrationCertificateImage;
      regData.leaseManagementContractImage =
        userDetails.leaseManagementContractImage;
      regData.riBBankAccountImage = userDetails.riBBankAccountImage;
      ...
      regData.numberOfPlace = userDetails.numberOfPlace;
      regData.vehicleColor = userDetails.vehicleColor;
      regData.options = userDetails.options;
      regData.companyName = userDetails.companyName;
      regData.companyAddress = userDetails.companyAddress;
      regData.companyLicenseNumber = userDetails.companyLicenseNumber;
      regData.queue = false;
