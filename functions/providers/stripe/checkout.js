...
const stripe = require("stripe")(
  "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"
);
...
module.exports.render_checkout = function (request, response) {
  ...
  const server_url = refr
    ? refr.includes("bookings")
      ? refr.substring(0, refr.length - 8)
      : refr
  ...
  stripe.checkout.sessions.create(session_data, (err, session) => {
    if (err) {
      response.send({ error: err });
    } else if (session) {
      response.send(templateLib.getTemplate(stripe_public_key, session.id));
    } else {
