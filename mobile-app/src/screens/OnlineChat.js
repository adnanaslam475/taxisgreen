...
export default function OnlineChat(props) {
  ...
  useEffect(() => {
    return () => {
      if (curBooking) {
        dispatch(updateMessagesToRead({ curBooking, role }));
      }
    };
  }, [curBooking]);
  const SendMsg = () => {
  ...
  const renderItem = ({ item }) => {
    return item.source == "rider" ? (
      <View style={styles.drivermsgStyle}>
        <Text
          style={[styles.msgTextStyle, { textAlign: isRTL ? "left" : "right" }]}
        >
          {item ? item.message : t("chat_not_found")}
        </Text>
        <Text
          style={[styles.msgTimeStyle, { textAlign: isRTL ? "left" : "right" }]}
        >
          {item ? item.msgTime : null}
        </Text>
      </View>
    ) : (
      <View style={styles.riderMsgStyle}>
        <Text
          style={[styles.riderMsgText, { textAlign: isRTL ? "right" : "left" }]}
        >
          {item ? item.message : t("chat_not_found")}
        </Text>
        <Text
          style={[styles.riderMsgTime, { textAlign: isRTL ? "right" : "left" }]}
        >
          {item ? item.msgTime : null}
        </Text>
      </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <Header
        ...
        centerComponent={
          <Text style={styles.headerTitleStyle}>{t("chat_title")}</Text>
        }
