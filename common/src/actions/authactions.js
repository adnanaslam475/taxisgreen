...
export const fetchUser = () => (dispatch) => (firebase) => {
  const { auth, config, singleUserRef, settingsRef } = firebase;
                  ...
                  setTimeout(() => {
                    singleUserRef(user.uid).once("value", (snapshot) => {
                      if (snapshot.val()) {
                        ...
                        //     payload: {
                        //       code: store.getState().languagedata
                        //         .defaultLanguage.auth_error,
                        //       message:
...
export const emailSignUp = (regData) => async (firebase) => {
  ...
  if (regData.usertype === "driver") {
    if (regData.parkingCardImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.parkingCardImage);
      regData.parkingCardImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (regData.idImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.idImage);
      regData.idImage = await driverDocsRef(timestamp).getDownloadURL();
    }
    if (regData.insuranceCertificateImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.insuranceCertificateImage);
      regData.insuranceCertificateImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (regData.riBBankAccountImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.riBBankAccountImage);
      regData.riBBankAccountImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (regData.certificateRcProImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.certificateRcProImage);
      regData.certificateRcProImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (regData.kbisOrInseeImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.kbisOrInseeImage);
      regData.kbisOrInseeImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (regData.taxiProCardImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.taxiProCardImage);
      regData.taxiProCardImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (regData.licenseImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.licenseImage);
      regData.licenseImage = await driverDocsRef(timestamp).getDownloadURL();
    }
    if (regData.leaseManagementContractImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.leaseManagementContractImage);
      regData.leaseManagementContractImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }

    if (regData.registrationCertificateImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(regData.registrationCertificateImage);
      regData.registrationCertificateImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
  }
          ...
          singleUserRef(user.user.uid).once("value", (snapshot) => {
            if (!snapshot.val()) {
              let userData = {
                ...
                referralId:
                  (user.additionalUserInfo.profile.first_name
                    ? user.additionalUserInfo.profile.first_name.toLowerCase()
                    : "temp") +
          ...
          singleUserRef(user.user.uid).once("value", (snapshot) => {
            if (!snapshot.val()) {
              let userData = {
                ...
                usertype: "rider",
                referralId:
                  "rider" + Math.floor(1000 + Math.random() * 9000).toString(),
                approved: true,
                walletBalance: 0,
                loginType: "apple",
              };
...
export const updateProfile =
  (userAuthData, updateData) => (dispatch) => async (firebase) => {
    ...
    if (updateData.parkingCardImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(updateData.parkingCardImage);
      updateData.parkingCardImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }

    if (updateData.insuranceCertificateImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(updateData.insuranceCertificateImage);
      updateData.insuranceCertificateImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (updateData.riBBankAccountImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(updateData.riBBankAccountImage);
      updateData.riBBankAccountImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (updateData.certificateRcProImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(updateData.certificateRcProImage);
      updateData.certificateRcProImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (updateData.kbisOrInseeImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(updateData.kbisOrInseeImage);
      updateData.kbisOrInseeImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (updateData.taxiProCardImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(updateData.taxiProCardImage);
      updateData.taxiProCardImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    ...
    if (updateData.leaseManagementContractImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(
        updateData.leaseManagementContractImage
      );
      updateData.leaseManagementContractImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
    if (updateData.idImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(updateData.idImage);
      updateData.idImage = await driverDocsRef(timestamp).getDownloadURL();
    }
    if (updateData.registrationCertificateImage) {
      let timestamp = new Date().toISOString();
      await driverDocsRef(timestamp).put(
        updateData.registrationCertificateImage
      );
      updateData.registrationCertificateImage = await driverDocsRef(
        timestamp
      ).getDownloadURL();
    }
...
export const updateProfileImage =
  (userAuthData, imageBlob) => (dispatch) => (firebase) => {
    ...
    profileImageRef(userAuthData.uid)
      .put(imageBlob)
      .then(() => {
        imageBlob.close();
