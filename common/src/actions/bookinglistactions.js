  ...
  bookingListRef(uid, role).on("value", (snapshot) => {
    if (snapshot.val()) {
      ...
      for (let i = 0; i < bookings.length; i++) {
        ...
        if (
          ["ACCEPTED", "ARRIVED", "STARTED", "CUSTOMER_ON_BOARD"].indexOf(
            bookings[i].status
          ) != -1 &&
          role == "driver"
        ) {
...
export const updateBooking = (booking) => (dispatch) => async (firebase) => {
  ...
  if (booking.status == "CUSTOMER_ON_BOARD") {
    singleBookingRef(booking.id).update(booking);
  }
  ...
  if (booking.status == "REACHED") {
    ...
    if (appcat == "taxi") {
      ...
      const trackingSnap = await trackingRef(booking.id)
        .orderByKey()
        .once("value");
      ...
      let grandTotal = booking.trip_cost;
      let convenienceFee = 0;
      if (rates.convenience_fee_type && rates.convenience_fee_type == "flat") {
        convenienceFee = rates.convenience_fees;
      } else {
        convenienceFee =
          (grandTotal * parseFloat(rates.convenience_fees)) / 100;
      }
      booking.drop = {
        add: address,
        lat: driverLocation.lat,
      ...
      booking.convenience_fees = convenienceFee;
      booking.driver_share = grandTotal - convenienceFee;
  ...
  if (booking.status == "PAID") {
    ...
    singleUserRef(booking.driver).once("value", (snapshot) => {
      ...
      walletBalRef(booking.driver).set(
...
export const cancelBooking = (data) => (dispatch) => (firebase) => {
  const { singleBookingRef, singleUserRef, requestedDriversRef } = firebase;
  ...
  singleBookingRef(data.booking.id)
    .update({
      status: "CANCELLED",
      reason: data.reason,
    ...
    .then(() => {
      if (
        ...
        (data.booking.status === "ACCEPTED" ||
          ...
          data.booking.status === "CUSTOMER_ON_BOARD" ||
...
export const updateBookingImage =
  (booking, imageType, imageBlob) => (dispatch) => (firebase) => {
    ...
    bookingImageRef(booking.id, imageType)
      .put(imageBlob)
      .then(() => {
        imageBlob.close();
